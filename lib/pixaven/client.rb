module Pixaven
    ...
    class Client
        ...
        %i(adjust auto border crop mask output padding resize response scale store stylize watermark webhook cdn).each do |method|
        ...
        def validate_request(type)
            ...
            raise Pixaven::Errors::RequestTypeConflict.new(@request.type, type)
        end
        ...
        def validate_binary_operations
            if @request.data[:webhook]
                raise Pixaven::Errors::OperationNotSupported, "Binary responses are not supported when using Webhooks"
            end

            if @request.data[:store]
                raise Pixaven::Errors::OperationNotSupported, "Binary responses are not supported when using External Storage"
            end
        end
        ...
        def validate_url(url)
            ...
            raise Pixaven::Errors::InvalidProxyUrl, "Proxy url is not valid"
        end
    end
end